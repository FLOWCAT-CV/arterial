...
def preprocessing_extracranial_vessels(case_dir, segmentation_nifti_path, master_volume_node, fast_segmentation = False):
    """
    ...
    voxel_size = np.prod(segmentation_nifti_path.header["pixdim"][1:4])
...
def preprocessing_intracranial_vessels(case_dir, segmentation_nifti, master_volume_node):
    """
    Performs segmentation of a binary mask using Slicer's segmentEditorWidget to create
    volume model of the segmented bodies. Also, it applies preprocessing of the resulting
    segmentation.
