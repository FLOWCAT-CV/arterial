...
def preprocessing_vessels(case_dir, master_volume_node, fast_segmentation = False):
    ...
    if fast_segmentation:
        ...
        _, _, _, _, min_is, max_is = get_bounding_box_limits_3d(masked_volume_array)
        masked_volume_array[int(np.round((max_is - min_is) * 0.9)):] = 0
...
def preprocessing_intracranial_vessels(case_dir, master_volume_node):
    """
    Performs segmentation of a binary mask using Slicer's segmentEditorWidget to create
    volume model of the segmented bodies. Also, it applies preprocessing of the resulting
    segmentation.
...
def preprocessing_thrombus(case_dir, master_volume_node):
    """
    Performs segmentation of a binary mask using Slicer's segmentEditorWidget to create
    volume model of the segmented bodies. Also, it applies preprocessing of the resulting
    segmentation.
    ...
    # Save segmentation as vtk
    vtk_writer = vtk.vtkPolyDataWriter()
    vtk_writer.SetFileName(os.path.join(case_dir, "thrombus_segmentation.vtk"))
    vtk_writer.SetInputConnection(decimator.GetOutputPort())
    vtk_writer.Write()
    # Save segmentation as stl
    stl_writer = vtk.vtkSTLWriter()
    stl_writer.SetFileTypeToBinary()
    stl_writer.SetFileName(os.path.join(case_dir, "thrombus_segmentation.stl"))
    stl_writer.SetInputConnection(decimator.GetOutputPort())
    stl_writer.Write()
